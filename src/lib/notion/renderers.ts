...
import {string_component} from '../../components/dynamic'
...
function applyTags(tags = [], children, noPTag = false, key) {
  ...
  for (const tag of tags) {
    ...
    if (tagName === 'e') {
      tagName = string_component.Equation
      props.displayMode = false
      child = tag[1]
    }

    child = React.createElement(string_component[tagName] || tagName, props, child)
  }
  return child
}
...
export function textBlock(text = [], noPTag = false, mainKey) {
  ...
  return React.createElement(
    noPTag ? React.Fragment : string_component.p,
    { key: mainKey },
    ...children,
    noPTag
  )
